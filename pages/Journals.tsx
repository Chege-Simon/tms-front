...
const Journals: React.FC = () => {
  ...
  const columns: Column<Journal>[] = useMemo(() => [
    ...
    { 
      header: 'Debit', 
      accessor: (j) => j.journal_type === JournalTypeEnum.DEBIT ? `${j.currency} ${parseFloat(j.amount || 0).toFixed(2)}` : '' 
    },
    { 
      header: 'Credit', 
      accessor: (j) => j.journal_type === JournalTypeEnum.CREDIT ? `${j.currency} ${parseFloat(j.amount || 0).toFixed(2)}` : '' 
    },
  ], []);
